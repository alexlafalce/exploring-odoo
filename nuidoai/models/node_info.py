...
def get_related_object(type, id, infos):
    ...
    if type == "service":
        obj = next((info for info in infos["services"] if info["id"] == id), None)
    elif type == "agent":
        obj = next((info for info in infos["agents"] if info["id"] == id), None)
    elif type == "plugin":
        obj = next((info for info in infos["plugins"] if info["id"] == id), None)
    elif type == "termination_strategy":
        obj = next((info for info in infos["termination_strategies"] if info["id"] == id), None)
    elif type == "selection_strategy":
        obj = next((info for info in infos["selection_strategies"] if info["id"] == id), None)
        ...
        if section_rec:
            ...
            if section_type == "object":
                ...
                if obj:
                    ...
                    raise f"Section role not found: {o}"
